var board;
let isTheGameOver = false;                 //To check if the game s over or not
let lastText = "";
...
function setGame() {
    ...
    // Erasing Game Over items
    let t = document.getElementById("gameOverText");
    if (t)
    t.remove();
    t = document.getElementById("restartText");
    if (t)
    t.remove();
    t = document.getElementById("restartButton");
    if (t)
    t.remove();
    t = document.getElementById("noButton");
    if (t)
    t.remove();
    // Erasing Game Over Items
...
document.addEventListener('keyup', (e) => {
        ...
        if ((flag == true) && (isTheGameOver == false)) {                                        //Game Over
            let b = document.getElementById("board"); 
            b.classList.add("blurBoard");
            let gameOverText = document.createElement("h1");        // Game over text addition
            gameOverText.classList.add("game-over");
            gameOverText.id = "gameOverText";

            const gameOverNode = document.createTextNode("Game Over");
            gameOverText.appendChild(gameOverNode);         // Game over text added
            let fullGameBoard = document.getElementById("game-board");
            fullGameBoard.appendChild(gameOverText);
            lastText = "Game Over";
            isTheGameOver = true;
        }
        document.getElementById("score").innerText = score + "\n" + lastText;
...
function undoMove() {                                //undo
    ...
    // If Game was over, erasing Game Over Items
    let t = document.getElementById("gameOverText");
    if (t)
    t.remove();
    t = document.getElementById("restartText");
